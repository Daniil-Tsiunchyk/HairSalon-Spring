document.addEventListener("DOMContentLoaded", function () {
  ...
  const resetCodeModal = new bootstrap.Modal(
  ...
  resetPasswordButton.addEventListener("click", function () {
    ...
    const response = fetch(
      "http://localhost:8080/api/users/reset-code?email=" + email,
      {
        method: "POST",
      }
    ).then((response) => {
  ...
  resetCodeForm.addEventListener("submit", function (event) {
    ...
    fetch(
      "http://localhost:8080/api/users/reset-code/check?email=" +
      email +
      "&resetCode=" +
      resetCode,
      {
      ...
      if (response.status === 200) {
        ...
        resetMessage.innerHTML =
          "Неверный код сброса пароля. Пожалуйста, попробуйте еще раз.";
      }
    });
  });
});
