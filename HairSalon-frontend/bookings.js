...
document
    ...
    .addEventListener("submit", function (event) {
        ...
        fetch("http://localhost:8080/api/bookings", {
            ...
            body: JSON.stringify({ user: { id: userId }, hairService: { id: serviceId }, barber: { id: barberID }, dateTime: now, status: "RESERVED" })
...
function showEditBooking(BookingId) {
    ...
    submitEditButton.onclick = function () {
        ...
        const now = new Date();
        console.log(JSON.stringify({ user: { id: userId }, hairService: { id: serviceId }, barber: { id: barberID }, dateTime: now, status: "RESERVED" }),
        )

        editBooking(BookingId, userId, serviceId, barberID, now)
    }
    modal.style.display = "block";
}

function editBooking(BookingId, userId, serviceId, barberID, now) {
    fetch(`http://localhost:8080/api/bookings/${BookingId}`, {
        method: "PUT",
        headers: {
            "Content-Type": "application/json",
